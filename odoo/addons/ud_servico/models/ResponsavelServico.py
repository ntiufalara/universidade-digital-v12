...
class ResponsavelAnaliseServico(models.Model):
    ...
    responsavel_id = fields.Many2one('res.users', 'Responsável', required=True)
    tipo = fields.Selection(utils.TIPO_RESPONSAVEL, 'Tipo', required=True)
    solicitacao_id = fields.Many2one('ud.servico.solicitacao', 'Solicitação', ondelete='restrict')

    _sql_constraints = [
        ('responsavel_id_unique', 'unique(responsavel_id)',
         'Este responsável já está cadastrado, use a lista para alterar o registro.')
    ]
