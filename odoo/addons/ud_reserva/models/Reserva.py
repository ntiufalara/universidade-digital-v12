...
class Reserva(models.Model):
    ...
    solicitante_id = fields.Many2one('res.users', string='Solicitante', required=True,
                                     default=lambda self: self.env.uid)
    titulo = fields.Char('Nome do evento', required=True)
    descricao = fields.Text('Descrição do evento')
    data_solicitacao = fields.Datetime('Data da solicitação', default=fields.datetime.now())
    state = fields.Selection(utils.STATUS_RESERVA, 'Status', default='nova')
    dia_ids = fields.One2many('ud.reserva.dia', 'reserva_id', 'Dias', ondelete='cascade')
    espaco_aprovado_ids = fields.Many2many('ud.espaco', 'reserva_espaco_aprovado_rel', string='Espaços aprovados')
    motivo_cancelamento = fields.Text('Motivo de cancelamento')
    data_cancelamento = fields.Datetime('Data/hora de cancelamento')
    ...
    def read(self, fields=None, load='_classic_read'):
        ...
        :return:
        """
        result = super(Reserva, self).read(fields, load)
    ...
    @api.constrains('dia_ids', 'solicitante_id', 'descricao', 'state')
    ...
    def load_from_openerp7_cron(self):
        ...
        :return:
        """
        _logger.info('Sincronizando reservas de espaço com o Openerp 7')
        import xmlrpclib
        ...
        reserva_ids = server.execute(db, uid, password, 'ud.reserva', 'search',
                                     [('hora_entrada', '>=', datetime.datetime(2018, 1, 1, 0, 0, 0).isoformat()),
                                      ('state', '!=', 'cancelada')])
